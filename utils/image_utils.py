...
def resize_image(img, new_size=(48, 48)):
    ...
    for i in range(new_h):
        ...
        if src_i >= old_h:  src_i = old_h - 1
        for j in range(new_w):
            src_j = int(j * col_ratio)
            if src_j >= old_w: src_j = old_w - 1
...
def rgba_to_grayscale(img_rgba):
    if img_rgba.ndim != 3 or img_rgba.shape[2] != 4:
        raise ValueError(f"rgba_to_grayscale expects (H,W,4), got {img_rgba.shape}")
    rgb = img_rgba[:, :, :3]
...
def bgr_to_grayscale(img_bgr):
    ...
    B = img_bgr[:, :, 0]; G = img_bgr[:, :, 1]; R = img_bgr[:, :, 2]
...
def load_and_process_image_array(img_array, size=(48, 48)):
    if img_array.ndim == 3:
        if img_array.shape[2] == 3:
            ...
            gray = rgba_to_grayscale(img_array)
        else:
            raise ValueError(f"Unsupported channels in image: {img_array.shape}")
...
def load_and_process_image(path, size=(48, 48)):
    img = plt.imread(path)
    return load_and_process_image_array(img, size=size)
