...
def bgr_to_grayscale(img_bgr):
    """
    Convert BGR image (from OpenCV) -> grayscale (H,W) using RGB weights.
    For BGR, channels are [B, G, R] at indices [0,1,2].
    """
    if img_bgr.ndim != 3 or img_bgr.shape[2] != 3:
        raise ValueError(f"bgr_to_grayscale expects (H,W,3), got {img_bgr.shape}")
    # Map BGR -> weighted RGB formula
    B = img_bgr[:, :, 0]
    G = img_bgr[:, :, 1]
    R = img_bgr[:, :, 2]
...
def _to_float01(arr):
    ...
    # Heuristic: if values exceed 1.5, assume 0..255 and scale
    if np.nanmax(a) > 1.5:
        a = a / 255.0
    # Clip to be safe
...
def load_and_process_image_array(img_array, size=(48, 48)):
    if img_array.ndim == 3 and img_array.shape[2] == 3:
        # Assume RGB order for file-based loaders unless it's BGR
...
def to_model_input_from_bgr(frame_bgr, size=(48, 48)):
    if frame_bgr.ndim == 3 and frame_bgr.shape[2] == 3:
        ...
        gray = frame_bgr  # already grayscale
    else:
